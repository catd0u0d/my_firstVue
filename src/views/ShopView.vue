...
const filteredProducts = computed(() => {
    // 邏輯處理 資料處理 最後結果用 return 回傳
    return products.value.filter((product) => {
        const matchKeyword =
            !keyword2.value || product.name.toLowerCase().includes(keyword2.value.toLowerCase())

        const matchCategory = !categoryId2.value || product.categoryId === categoryId2.value

        return matchKeyword && matchCategory
    })
})
// const list = computed(() =>{
//     return '$NT ' + keyword2.value + '元'
// })
...
<template>
    ...
    <div class="d-flex gap-2 mb-4">
        <input 
            ...
            v-model="keyword2" 
        />
        <select class="form-select" style="max-width: 160px" v-model="categoryId2">
            <option value="">全部分類</option>
    ...
    <!-- 商品列表 -->
    <div class="row g-3">
        <div>共 {{ filteredProducts.length }} 筆結果</div>
        <!-- 商品卡片 -->
        <!-- for 迴圈加在這邊 -->
        <div class="col-md-4" v-for="(item, index) in filteredProducts" :key="item.id">
            <div class="card h-100">
                <div class="card-body d-flex flex-column">
                    <h6 class="card-title fw-bold">
                        <RouterLink
                            :to="{ name: 'shop-product', query: { id: item.id } }"
